...
public class RabbitmqConfig {
    ...
    public Queue successEmailQueue(){
        return new Queue(env.getProperty("mq.success.email.queue"),true);
    }

    @Bean
    public TopicExchange successEmailExchange(){
        return new TopicExchange(env.getProperty("mq.success.email.exchange"),true,false);
    }

    @Bean
    public Binding successEmailBinding(){
        //进行绑定：
        return BindingBuilder.bind(successEmailQueue()).to(successEmailExchange()).with(env.getProperty("mq.success.email.routing.key"));
    }
